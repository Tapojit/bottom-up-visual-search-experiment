...
import random, datetime, os, json, math
...
def draw_frame(SETTINGS, frame_parameters, filename):
	...
	for n, color, cutoff in zip(range(9), frame_parameters["all colors"], frame_parameters["all cutoffs"]):
		coll = center[0] + (SETTINGS["diamond spread"] * min(*SETTINGS["frame size"]) * math.sin(math.pi * 2 * n / 9))
		row = center[1] + (SETTINGS["diamond spread"] * min(*SETTINGS["frame size"]) * math.cos(math.pi * 2 * n / 9))
