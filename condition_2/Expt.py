...
import gtk
from data.blocks import convert_blocks
...
#        SETTINGS["diamond scale"]=SETTINGS["diamond scale"]*scaleVar
    ...
    worksheet.write('B1', 'orientation')
    worksheet.write('C1', 'corrAns')
    worksheet.write('D1', 'Distractor1')
    worksheet.write('E1', 'Distractor2')
    worksheet.write('F1', 'Distractor3')
    worksheet.write('G1', 'Distractor4')
    worksheet.write('H1', 'Distractor5')
    worksheet.write('I1', 'Distractor6')
    worksheet.write('J1', 'Distractor7')
    worksheet.write('K1', 'Distractor8')
    worksheet.write('L1', 'Distractor9')
    worksheet.write('M1', 'Sub_block')
    ...
    for i in range(count_frames):
        ...
        if (dataJson["target cut on"][i]=="left"):
            ...
            key="slash"
            orienta="hor"
        worksheet.write_string(row,column,str(dataJson["target index"][i]))
        worksheet.write_string(row,column+1,orienta)
        worksheet.write_string(row,column+2,key)
        for x in range(9):
            if(x!=dataJson["target index"][i]):
                if (dataJson["all cutoffs"][i][x]=="left"):
                    orienta2="ver"
                else:
                    orienta2="hor"
                worksheet.write_string(row,column+3+x,orienta2)
            else: worksheet.write_string(row,column+3+x,"nil")
        if (row<37):
            worksheet.write_string(row,column+12,str(1))
        else:
            worksheet.write_string(row,column+12,str(2))
        row+=1
